...
class Hearts:
	...
	def handleScoring(self):
		...
		for player in self.players:
			...
			self.losingPlayer = p
		self.scoreRank[0]=self.losingPlayer.score
		self.scoreRank[3]=self.getWinner().score
		secondAndThird=[]
		scores=[]
		for player in self.players:
		...
		c=Counter(scores)
		for player in self.players:
			if player.score<self.scoreRank[0] and player.score>self.scoreRank[3]:
				secondAndThird.append(player.score)
			elif player.score==self.scoreRank[0] and c[player.score]>1:
				secondAndThird.append(player.score)
			elif player.score==self.scoreRank[3] and c[player.score]>1:
				secondAndThird.append(player.score)
		if len(secondAndThird) > 1:
			for i in secondAndThird:
				print i
	...
	for i in range(simNum):
		...
		for i in range(0,4):
			if hearts.scoreRank.index(hearts.players[i].score) is 3:
				...
				totalScore[i][1]+=1
			elif hearts.scoreRank.index(hearts.players[i].score) is 1:
				totalScore[i][2]+=1
			else:
				totalScore[i][3]+=1

	for i in range(0,4):
		print i.__str__()+"first: "+totalScore[i][0].__str__()
		print i.__str__()+"second: "+totalScore[i][1].__str__()
